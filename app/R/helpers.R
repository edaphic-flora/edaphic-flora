...
# Authoritative state invasive species pages, used to link badges to source-of-truth lists.
STATE_INVASIVE_URLS <- c(
...
get_state_invasive_url <- function(state_code) {
  if (is.null(state_code) || !nzchar(state_code)) return(NULL)
  url <- STATE_INVASIVE_URLS[[toupper(state_code)]]
  if (is.null(url) || is.na(url)) NULL else url
}
